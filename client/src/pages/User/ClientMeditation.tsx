...
export const ClientMeditation = () => {
    ...
    useEffect(() => {
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            try {
                ...
                if (data.type === 'rutube-player-state' && data.currentTime && data.duration) {
                    ...
                    if (progress >= 90 && meditation?.accessType !== 'free') {
        ...
        // Альтернативный метод: периодическая проверка через iframe
        rutubeProgressIntervalRef.current = window.setInterval(async () => {
            try {
                const iframe = rutubeIframeRef.current;
                if (!iframe || !iframe.contentWindow) return;

                // Пытаемся получить информацию о видео через postMessage
